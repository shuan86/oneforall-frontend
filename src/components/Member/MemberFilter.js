...
const Filter = ({ setMemberFliter }) => {
  ...
  useEffect(() => {
    ...
    console.log(`isReviewer:${isReviewer}isPublisher:${isPublisher}`);
    if(!isReviewer){
  ...
  const onClickFilter = (memberType) => {
    if (memberType == 'vistor') {//vistor
      setMemberFliter([true,false,false]);
      if(!isReviewer) {
        if(!isPublisher){
          setBgColor(["var(--navy-blue)", "", ""]);
          setFtColor(["var(--white)", "var(--black-alpha-20)", "var(--black-alpha-20)"]);
          return
        }
      };
      if(isReviewer) {
    ...
    } else if (memberType == 'reviewer') {//reviewer
